...
public enum OreTypeOverworld {
    ...
    OreTypeOverworld(Lazy<Block> block, int maxVeinSize, int minHeight, int maxHeight) {
        this.block = block;
    ...
    public static OreTypeOverworld get(Block block) {
        for (OreTypeOverworld ore : values()) {
            if(block == ore.block) {
                return ore;
            }
        }
